...
describe('\n\nVerify permission', function(){
  ...
  describe('\n\nGET /users', function(){
    ...
    it('users is forbidden when authenticated as normal user', function (done) {
    ...
    it('promotions returns "no token" when not properly authenticated', function (done) {
      request(app)
        .get('/promotions')
        .expect(HTTP_FORBIDDEN)
    ...
    it('post a dish when authenticated as admin', function(done){
      login("admin", "password", function(auth_res) {
        ...
        request(app)
          ...
          .expect(HTTP_CREATED)
          .expect(/Added the promotion with id:/)
          /*.expect(function(res) {
    ...
    it('is forbidden when authenticated as normal user', function(done){
      login("sylvain", "password", function(auth_res) {
        ...
        request(app)
          ...
          .set('x-access-token', token)
          .expect(HTTP_FORBIDDEN)
          .expect(/You are not authorized to perform this operation!/)
          /*.expect(function(res) {
            assert.deepEqual(res.body, {});
          })*/
          .end(done);
